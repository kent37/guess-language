...
from .guess_language import (UNKNOWN, createOrderedModel, find_runs,
                             guessLanguage, guessLanguageId, guessLanguageInfo,
                             guessLanguageName, guessLanguageTag, normalize)
...
class guess_language_test(unittest.TestCase):
    ...
    def test_find_runs(self):
        self.assertEqual(
            ["Basic Latin"], find_runs("This is a test of the language checker")
        )
        self.assertEqual(
            set(["Basic Latin", "Extended Latin"]), set(find_runs("abcdééé"))
        )
    ...
    def test_createOrderedModel(self):
        ...
        self.assertEqual(
            ["cab", "abc", "abd", "bca", "bdc", "dca"], createOrderedModel("abcabdcab")
        )
    ...
    def test_guess(self):
        tests = [
            ...
            (
                "トヨタ自動車、フィリピンの植林活動で第三者認証取得　トヨタ自動車(株)（以下、トヨタ）は、2007年９月よりフィリピンのルソン島北部に位置するカガヤン州ペニャブラン",
                "ja",
            ),
        ]
